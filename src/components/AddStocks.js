  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentStock) {
        ...
        response = await axios.put(`/api/stocks/${formData.stockId}`, {
      ...
      // Enhanced error handling based on the backend validation
      if (err.response) {
        switch (err.response.status) {
          case 404:
            ...
            break;
          case 400:
            if (err.response.data.message.includes('Stock ID already exists')) {
              setError('This Stock ID is already in use. Please choose a unique Stock ID.');
            } else if (err.response.data.message.includes('Stock already exists for this product')) {
              setError('A stock entry already exists for this product. Use update to modify existing stock.');
            } else {
              setError(err.response.data.error || 'Invalid stock creation request');
            }
            break;
          default:
