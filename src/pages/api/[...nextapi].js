...
async function handleAddStock(req, res) {
  ...
  try {
    ...
    const [stockIdCheck] = await db.query(
      'SELECT * FROM product_stocks WHERE id = ?',
      [id]
    );

    if (stockIdCheck.length > 0) {
      return res.status(400).json({ 
        error: 'Stock ID already exists',
        message: 'Cannot create a new stock entry with an existing stock ID'
      });
    }

    if (existingStock.length > 0) {
      // Stock for this product already exists, prevent creating a new entry
      return res.status(400).json({ 
        error: 'Stock ID already exists for this product',
        message: 'Use stock update endpoint to modify existing stock'
      });
    }
